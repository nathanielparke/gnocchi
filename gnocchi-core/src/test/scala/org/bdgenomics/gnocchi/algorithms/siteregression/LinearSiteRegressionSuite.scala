...
class LinearSiteRegressionSuite extends GnocchiFunSuite {
  ...
  ignore("LinearSiteRegression.prepareDesignMatrix should produce a matrix with missing values filtered out.") {
    ...
    observations(0) = ("1/0", Array[Double](1))
    observations(1) = ("0/1", Array[Double](2))
    observations(2) = ("0/1", Array[Double](3))
    observations(3) = ("0/0", Array[Double](4))
    observations(4) = ("1/1", Array[Double](5))
    observations(5) = ("1/.", Array[Double](6))
    observations(6) = ("./.", Array[Double](7))
    observations(7) = ("./0", Array[Double](8))
    observations(8) = ("./.", Array[Double](9))
    observations(9) = ("./1", Array[Double](10))

    val (genotypes, phenotypes) = observations.unzip
    val genotypeStates = genotypes.toList.map(_.split("/|\\|")).zipWithIndex.map(item => GenotypeState(item._2.toString, item._1.count(_ == "0").toByte, item._1.count(_ == "1").toByte, item._1.count(_ == ".").toByte))
    val cv = CalledVariant("rs123456", 1, 1, "A", "C", genotypeStates)
