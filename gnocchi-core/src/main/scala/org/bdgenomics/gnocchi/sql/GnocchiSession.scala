...
class GnocchiSession(@transient val sc: SparkContext) extends Serializable with Logging {
   ...
   * @return A Map of phenotype name to Phenotype object
   */
  def loadPhenotypesOnDriver(phenotypesPath: String,
                             primaryID: String,
                             phenoName: String,
                             delimiter: String,
                             covarPath: Option[String] = None,
    ...
    if (covarPath.isDefined) {
      phenoMap.map(pair => {
        ...
        sampleId -> Phenotype(sampleId, phenoName, pheno.toDouble, covarList.get)
      })
    } else {
      phenoMap.map(pair => {
        val (sampleId, pheno) = pair
        sampleId -> Phenotype(sampleId, phenoName, pheno.toDouble, List())
      })
    }
  }
