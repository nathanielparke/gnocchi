...
class RegressPhenotypes(protected val args: RegressPhenotypesArgs) extends BDGSparkCommand[RegressPhenotypesArgs] {
  ...
  def run(sc: SparkContext) {
    ...
    val phenotypes = if (args.covarFile != null) {
      sc.loadPhenotypesOnDriver(args.phenotypes, args.sampleUID, args.phenoName, phenoDelimiter, Option(args.covarFile), Option(args.covarNames.split(",").toList), covarDelimiter = covarDelimiter, missing = missingPhenos)
    } else {
      sc.loadPhenotypesOnDriver(args.phenotypes, args.sampleUID, args.phenoName, phenoDelimiter, missing = missingPhenos)
    }
